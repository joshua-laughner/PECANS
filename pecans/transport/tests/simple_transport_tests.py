...
class IdealTransportTests(unittest.TestCase):
    ...
    def test_simple_1d_transport(self):
        ...
        domain, config = _make_test_domain('simple_transport_test.cfg')
        x, _, _ = domain_utilities.compute_coordinates_from_config(config)
        ...
        wind_dict = config.get('TRANSPORT', 'wind_speeds')
        x_wind = wind_dict['x']
        if wind_dict['y'] != 0.0 or wind_dict['z'] != 0.0:
            raise RuntimeError('y and z winds should be 0')

        diffusion_coeff_dict = config.get('TRANSPORT', 'diffusion_coeffs')
        ...
        init_condition_opts = config.get('CHEMISTRY', 'initial_cond_opts')
        mu = init_condition_opts['center']
        sigma = init_condition_opts['width']
